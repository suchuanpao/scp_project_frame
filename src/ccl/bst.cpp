            ...
            struct libavl_allocator *allocator)
{
  ...
  tree = (struct bst_table *)allocator->libavl_malloc (allocator, sizeof *tree);
  if (tree == NULL)
    return NULL;
...
bst_probe (struct bst_table *tree, void *item)
{
  ...
  n = (struct bst_node*)tree->bst_alloc->libavl_malloc (tree->bst_alloc, sizeof *p);
  if (n == NULL)
    return NULL;
...
bst_t_insert (struct bst_traverser *trav, struct bst_table *tree, void *item)
{
  ...
  trav->bst_node = *q = (struct bst_node*)tree->bst_alloc->libavl_malloc (tree->bst_alloc,
                                                        sizeof **q);
  if (*q == NULL)
    {
      trav->bst_node = NULL;
...
void *
bst_t_replace (struct bst_traverser *trav, void *new0)
{
  void *old;

  assert (trav != NULL && trav->bst_node != NULL && new0 != NULL);
  old = trav->bst_node->bst_data;
  trav->bst_node->bst_data = new0;
  return old;
}

static void
copy_error_recovery (struct bst_node **stack, int height,
                     struct bst_table *new0, bst_item_func *destroy)
{
  assert (stack != NULL && height >= 0 && new0 != NULL);

  for (; height > 2; height -= 2)
    stack[height - 1]->bst_link[1] = NULL;
  bst_destroy (new0, destroy);
}
          ...
          bst_item_func *destroy, struct libavl_allocator *allocator)
{
  ...
  struct bst_table *new0;
  const struct bst_node *x;
  struct bst_node *y;

  assert (org != NULL);
  new0 = bst_create (org->bst_compare, org->bst_param,
                    allocator != NULL ? allocator : org->bst_alloc);
  if (new0 == NULL)
    return NULL;
  new0->bst_count = org->bst_count;
  if (new0->bst_count == 0)
    return new0;

  x = (const struct bst_node *) &org->bst_root;
  y = (struct bst_node *) &new0->bst_root;
  for (;;)
    {
      while (x->bst_link[0] != NULL)
        {
          if (height >= 2 * (BST_MAX_HEIGHT + 1))
            {
              ...
              copy_error_recovery (stack, height, new0, destroy);
          ...
          y->bst_link[0] =
            (bst_node *)new0->bst_alloc->libavl_malloc (new0->bst_alloc,
                                           sizeof *y->bst_link[0]);
          if (y->bst_link[0] == NULL)
            {
              if (y != (struct bst_node *) &new0->bst_root)
                {
                  y->bst_data = NULL;
                  y->bst_link[1] = NULL;
                }
